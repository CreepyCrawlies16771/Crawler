 ...
 * <pre>{@code
 * static {
 *     CrawlerRobotRegistry.setProvider(new CrawlerRobotProvider() {
 ...
 *             return MyRobot.builder(hwMap).withConfig(config).build();
 *         }
 *     });
 * }
 * }</pre>
...
public final class CrawlerRobotRegistry {
    ...
    /**
     * Registers the team's robot builder. Call once, typically from a static block in
     * your robot class.
     */
    public static void setProvider(CrawlerRobotProvider provider) {
    ...
    public static CrawlerRobot create(HardwareMap hwMap, CrawlerRobot.Config config) {
        return requireProvider().create(hwMap, config);
    }
